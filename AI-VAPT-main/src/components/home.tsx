...
const Home = () => {
  ...
  const generateProfessionalVulnerabilities = (
    ...
    if (targetAnalysis.neuralPatterns?.anomalyScore > 0.3) {
      discoveryRate += 0.15;
      depthRate += 0.12;
    }

    // Deep learning insights adjustments
    if (targetAnalysis.deepLearningInsights?.securityMaturity < 0.5) {
      discoveryRate += 0.2;
      depthRate += 0.15;
    }

    // ML threat profile adjustments
    if (targetAnalysis.mlThreatProfile?.attackProbability > 0.7) {
      discoveryRate += 0.18;
      depthRate += 0.14;
    }
...
`;
  ...
  const downloadPDFReport = async (content: string, target: string) => {
    // Create a simple text-based report as fallback, defined outside try block for scope access
